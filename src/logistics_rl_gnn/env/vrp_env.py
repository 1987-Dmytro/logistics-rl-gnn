"""VRPEnv — a gymnasium environment for dynamic CVRPTW (Phase 3 core).

The instance comes from config.instance.generate_instance (real pharmacy windows). Vehicles are
built SEQUENTIALLY; each starts and finishes at the depot (cur_time resets to 0), bounded by
capacity Q and tour time T_max. An action = the next node (0 = depot / close the route).
Feasibility goes through action_mask. The reward is sparse and terminal (dense shaping is an
option). THE TIME UNIT INSIDE THE ENVIRONMENT IS MINUTES.
"""
...
# features per node: x, y, demand, e, l, service, visited, feasible, congestion
_NF = 9  # congestion = traffic/incident level at the node (0 under free-flow, Phase 6b Step 0)


class VRPEnv(gym.Env):
    """CVRPTW environment. Action Discrete(k), k = depot + the instance's pharmacies."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dow = int(delivery_weekday)  # for time-context (congestion phase of the weekday)
        ...
        self.k = len(probe.node_ids)  # stops; fixed per weekday (seed does not change exclusions)
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                # time-context: [sin_h, cos_h, sin_dow, cos_dow] — congestion phase (6b Step 0)
    ...
    def _load(self, inst) -> None:
        self._inst = inst  # for the single scorer at the terminal step
        self.time_m = np.asarray(inst.time_matrix, dtype=float) / 60.0  # s→min
        self.dist_km = np.asarray(inst.dist_matrix, dtype=float) / 1000.0  # m→km
        self.win = np.asarray(inst.windows, dtype=float) / 60.0  # s→min
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.visited[0] = True  # the depot is not a customer
        self.steps = 0
        self.vehicles_used = 0
        self.total_km = 0.0
        self.total_time_min = 0.0
        self.total_late_min = 0.0
        self._emitted = 0.0  # sum of per-step rewards handed out (dense); terminal pays the rest
    ...
    def abs_minute(self) -> float:
        """Absolute minute of the day for the congestion phase: travel-model offset + tour time."""
    ...
    def _feasible(self, j: int) -> bool:
        ...
        if arrival > self.win[j, 1]:  # later than the late window l_j
            return False
        start = max(arrival, self.win[j, 0])  # wait until e_j when arriving early
        t_after = start + self.service_min[j]
        return t_after + self.travel.time(j, 0, t_after) <= self.t_max_min  # must make it back

    def _mask(self) -> np.ndarray:
        m = np.zeros(self.k, dtype=np.int8)
        m[0] = 1  # the depot is always allowed
    ...
    def _obs(self):
        ...
        nf[:, 6] = self.visited
        nf[:, 7] = mask
        nf[:, 8] = self.travel.node_congestion(self.coords, self.cur_time)  # 0 under free-flow
    ...
    def _close_route(self, returning: bool) -> None:
        """Finalise the current route. returning → append the trip back to the depot."""
        if returning and self.pos != 0:
            ...
            self.pos = 0
        if len(self.current_route) > 2:  # longer than [0, 0] → served ≥1 pharmacy
    ...
    def step(self, action):
        a = int(action)
        mask = self._mask()
        if mask[a] == 0:  # invalid action → penalty + terminal
            return self._obs(), -self.p_invalid, True, False, self._info({"invalid_action": a})

        step_km = step_time = 0.0
        if a == 0:  # close the route / return to the depot
            self._close_route(returning=True)
            self.vehicle_idx += 1
            if self.vehicle_idx < self.K:  # launch the next vehicle from the depot
                self.pos = 0
                self.cur_time = 0.0
                self.rem_cap = self.Q
                self.current_route = [0]
        else:  # drive to pharmacy a
            dt = self.travel.time(self.pos, a, self.cur_time)
            arrival = self.cur_time + dt
            wait = max(0.0, self.win[a, 0] - arrival)
            self.total_late_min += max(0.0, arrival - self.win[a, 1])  # ≈0 under the mask
        ...
        if bool(self.visited[1:].all()):  # every pharmacy served → final return
            self._close_route(returning=True)
            terminated = True
        elif self.vehicle_idx >= self.K:  # vehicles exhausted while customers remain
            terminated = True
        ...
        if terminated or truncated:
            ...
            self._emitted += reward  # accumulate what was handed out → terminal pays the rest
        else:
            reward = 0.0
        return self._obs(), float(reward), terminated, truncated, self._info()

    def _terminal_reward(self) -> float:
        # the single scorer is the source of truth for sparse AND dense (same formula as baselines).
        # dense hands out a variable amount per step → the terminal pays the rest (total − emitted),
        # so the episode reward sums to total in either mode; sparse: _emitted=0 → terminal=total.
    ...
    def sample_valid_action(self, obs) -> int:
        """A random allowed action from action_mask (through self.np_random)."""
